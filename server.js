...
app.get('/a', async (req, res) => {
  try {
    // Busca todos os atrativos do banco de dados
    const atrativos = await prisma.atrativo.findMany({
      include: { destinos: true } // Inclui os dados do destino relacionado
    });
    ...
    atrativos.forEach(atrativo => {
      html += `
        <div class="atrativo">
          <h1>${atrativo.name}</h1>
          <p><strong>Tipo:</strong> ${atrativo.tipo}</p>
          <p><strong>Descrição:</strong> ${atrativo.description || 'N/A'}</p>
          <p><strong>Dicas:</strong> ${atrativo.dicas || 'N/A'}</p>
          <p><strong>Destino:</strong> ${atrativo.destinos.name}</p>
        </div>
      `;
    });
